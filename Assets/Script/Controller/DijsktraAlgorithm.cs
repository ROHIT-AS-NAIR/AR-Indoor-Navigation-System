...
public class DijsktraAlgorithm : MonoBehaviour {
	...
	{		
		GameObject floorObject = startNode.GetComponent<NodeData>().GetParentObjectData().gameObject;
		...
		foreach (Transform node in floorObject.transform)
		{
			unVisitedList.Add(node.gameObject);
		}
		...
		while(currentNode != finishNode && (unVisitedList.Count>=0) ) //unVisitedList.Count.CompareTo(0)  ||  (unvisitedLeft > 0)   
		{
			...
			foreach (GameObject adjacentObject in currentNodeData.adjacentNodeList)
			{
				...
				if(adjacentNodeData.GetParentObjectData().roomName == finishNode.GetComponent<NodeData>().GetParentObjectData().roomName) { // if adjacent is final node  (adjacentObject == finishNode)
